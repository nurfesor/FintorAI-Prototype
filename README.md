...
**FintorAI** — мой ранний рабочий прототип персонального финансового ассистента, разработка которого началась в **июле 2025 года**. Именно из этой идеи и практического прототипа позже вырос текущий **Qarjym**.
...
Со временем эта идея выросла в отдельный, значительно более крупный проект — **Qarjym**. Поэтому FintorAI можно считать ранним прототипом и прямым предшественником текущего Qarjym: именно здесь проверялись базовые идеи быстрого ввода расходов, AI-разбора операций, категорий, истории и синхронизации финансовых данных.
...
### Подготовка Telegram, OpenAI и Google Sheets

1. Создайте собственного Telegram-бота через **@BotFather** и укажите его токен в `TELEGRAM_BOT_TOKEN`.
2. Укажите собственный OpenAI API key в `OPENAI_API_KEY`.
3. В Google Cloud включите **Google Sheets API**, создайте service account, скачайте его JSON credentials и сохраните файл локально, например как `credentials.json` (файл игнорируется Git).
4. Создайте Google Spreadsheet и поделитесь им с `client_email` из service-account JSON с правом **Editor**.
5. В таблице создайте две вкладки с точными названиями:
   - `Операции` — столбцы A:F: `Дата`, `Категория`, `Группа`, `Подкатегория`, `Сумма`, `Описание`;
   - `Справочник` — столбцы A:C: `Категория`, `Группа`, `Подкатегория`.
6. Укажите ID таблицы в `SPREADSHEET_ID`, а путь к JSON — в `GOOGLE_CREDENTIALS_JSON_PATH`.

`Справочник` может быть пустым: в таком случае бот использует встроенный fallback-набор категорий. Для записи операций вкладка `Операции` должна существовать.
...
FintorAI — запускаемый ранний прототип и прямой предшественник текущего Qarjym при корректно настроенных собственных credentials. Он не позиционируется как production-ready финансовый сервис.
